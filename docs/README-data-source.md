...
[`README.md`](../README.md) §1–§3; this file is the appendix that keeps the *numbers* and
...
separates *refusal* from *malformed request*:
...
because operations that *are* permitted return a descriptive `422` on the same token in
the same second.
...
makes *handle* discovery viable: search engines index X **profiles** well and X **posts**
